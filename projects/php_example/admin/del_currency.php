...
<p><strong>Выберите курс для удаления</strong></p>

<form action="drop_currency.php" method="post">
        
        <?php $result = mysqli_query($db,"SELECT currency,course,multiplicity,date,id FROM exchange_rates"); 
			  ...
			  printf ("<p><input name = 'id' type = 'radio' value = '%s'> <lable>%s %s %s</lable></p>",$myrow["id"],$myrow["date"],$myrow["currency"],$myrow["course"]);
		...
		?>
        <p><input name="submit" type="submit" value="Удалить курс" /></p>
        </form>
